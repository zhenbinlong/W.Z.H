    ...
    <div id="window" style="display:none;">
        <div class="page page-front">
            <div class="page-content">
                <form method="post" action="/Admin/round/update/{{$run->rid}}" enctype="multipart/form-data">
                    <div class="input-row">
                        <input type="hidden" name="_token" value="{{csrf_token()}}" />
                        <label class="label fadeIn">图片名称</label>
                        
                        <input type="text" name="rname" value="{{$run->rname}}" class="input fadeIn delay1"/>
                    </div>
                    <div class="input-row">
                        <label class="label fadeIn">图片</label>
                        <img src="{{$run->rpicture}}" width='100px' height='100px'>
                        <input id="username" type="file" name="rpicture" value="{{$run->rpicture}}" data-fyll="pineapple" class="fadeIn delay1"/>
                    </div>

                    <div class="input-row">
                        <label class="label fadeIn delay2">是否使用:<input type="radio" name="ruse" value="{{$run->ruse}}"  checked id="启用"/>启用
                                                                <input type="radio" name="ruse" value="{{$run->ruse}}" id="禁用" />禁用
                        </label>    
                    </div>
